# Generated by Powerlevel10k configuration wizard on 2025-05-03 at 11:05 CEST.
...
() {
  ...
  function prompt_my_pkgmanager() {
  local pkg_manager=$(detect_package_manager)
  if [[ -n $pkg_manager ]]; then
    p10k segment -t "$pkg_manager" -f "#7EBAE4"
  fi
}
  ...
  function detect_package_manager() {
    ...
    echo " nix"  # NixOS icon (using nerd-fonts)
    elif command -v dnf &> /dev/null; then
    echo " dnf"  # Fedora icon
    elif command -v apt &> /dev/null; then
    echo " apt"  # Debian icon
    elif command -v pacman &> /dev/null; then
    echo " pacman"  # Arch icon
    elif command -v emerge &> /dev/null; then
    echo " portage"  # Gentoo icon
    elif command -v brew &> /dev/null; then
    echo "🍺 brew"  # Homebrew icon
    elif command -v snap &> /dev/null; then
    echo " snap"  # Snap icon
    else
    echo "📦 pkg"  # Fallback icon
    fi
}
